import { JordanData } from "./productData.js";
...
JordanData.slice(0, 9).forEach((it) => {
  loadProductFromDataTab1(it);
});
JordanData.slice(9).forEach((it) => {
...
sortSelect.addEventListener("change", function () {
  ...
  if (sortSelect.value === "Mới nhất") {
    ...
    let newJordanData = JordanData.sort((a, b) => (a.name > b.name ? 1 : -1));
    newJordanData.forEach((item) => {
    ...
    let newJordanData = JordanData.sort((a, b) => (a.price > b.price ? 1 : -1));
    newJordanData.forEach((item) => {
      loadProductFromDataTab1(item);
    });
    handleProductDetail();
