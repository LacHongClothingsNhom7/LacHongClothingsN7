import { AdidasData } from "./productData.js";
...
AdidasData.slice(0, 9).forEach((it) => {
  loadProductFromDataTab1(it);
});
AdidasData.slice(9).forEach((it) => {
...
sortSelect.addEventListener("change", function () {
  ...
  if (sortSelect.value === "Mới nhất") {
    ...
    let newAdidasData = AdidasData.sort((a, b) => (a.name > b.name ? 1 : -1));
    newAdidasData.forEach((item) => {
    ...
    let newAdidasData = AdidasData.sort((a, b) => (a.price > b.price ? 1 : -1));
    newAdidasData.forEach((item) => {
      loadProductFromDataTab1(item);
    });
    handleProductDetail();
